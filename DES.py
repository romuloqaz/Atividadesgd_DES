...
# Tabelas de Permutação e Sboxes
IP = (
...
def encrypt(msg, key, decrypt=False):
    ...
    # aplique a função round 16 vezes no esquema seguinte (cifra feistel):
    L_last = L0
    R_last = R0
    for i in range(1,17):
        if decrypt: # basta usar as chaves redondas na ordem inversa
            i = 17-i
        L_round = R_last
...
def round_function(Ri, Ki):
    # expanda Ri de 32 para 48 bits usando a tabela E
    Ri = permutation_by_table(Ri, 32, E)
 
    # xor com tecla redonda
    Ri ^= Ki
 
    # dividir o Ri em 8 grupos de 6 bits
    Ri_blocks = [((Ri & (0b111111 << shift_val)) >> shift_val) for shift_val in (42,36,30,24,18,12,6,0)]
 
    # interpretar cada bloco como endereço para os S-boxes
    for i, block in enumerate(Ri_blocks):
        # pegue os bits que precisamos
        row = ((0b100000 & block) >> 4) + (0b1 & block)
        col = (0b011110 & block) >> 1
        # sboxes são armazenados como uma tupla unidimensional, então precisamos calcular o índice dessa maneira
        Ri_blocks[i] = Sboxes[i][16*row+col]
 
    # embalar os blocos juntos novamente por concatenar
    Ri_blocks = zip(Ri_blocks, (28,24,20,16,12,8,4,0))
    Ri = 0
    for block, lshift_val in Ri_blocks:
        Ri += (block << lshift_val)
 
    # outra permutação 32bit -> 32bit
    Ri = permutation_by_table(Ri, 32, P)
 
    return Ri
 
def permutation_by_table(block, block_len, table):
    # fundição rápida e suja para str
...
def generate_round_keys(C0, D0):
    # retorna dict de 16 chaves (uma para cada rodada)
    ...
    # criar mais 16 pares de chaves diferentes
    ...
    # round_keys [1] para a primeira rodada
    #           [16] para a 16ª rodada
    # não precisa mais de round_keys [0], remova
    del round_keys[0]
 
    # agora formam as chaves de CiDi 1 concatenado <= i <= 16 e aplicando PC2
